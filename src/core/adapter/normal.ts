...
export class AdapterQQBotNormal extends AdapterQQBot {
  ...
  async sendQQMsg (
    ...
    for (const v of elements) {
      if (v.type === 'text') {
        const { text, qrs } = await this.hendleText(v.text)
        list.content.push(text)
        // 将所有二维码添加到图片列表，分别发送（不使用ffmpeg）
        if (qrs.length > 0) {
          list.image.push(...qrs)
        }
        continue
  ...
  async sendGuildMsg (
    ...
    for (const v of elements) {
      if (v.type === 'text') {
        const { text, qrs } = await this.hendleText(v.text)
        list.content.push(text)
        // 频道场景使用 imageFiles，base64 图片需要通过 FormData 上传
        if (qrs.length > 0) {
          qrs.forEach(qr => {
            list.imageFiles.push(qr)
          })
        }
        continue
      }
